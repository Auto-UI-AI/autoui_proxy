...
import { callOpenRouterJSON, callOpenRouterStream, type ChatRequest } from "../../llm/openaiCompat.js";
...
export class ChatController {
    ...
    async handleExtraAnalysisNonStream(req: Request, body: ChatRequest , tokenEntity: any) {
        const policy = await this.policies.resolve(body.appId);
        if (!policy) return { status: 403, json: { error: "Unknown appId" } };
        const ip = getClientIp(req);
        const rlKey = tokenEntity?._id ? `token:${String(tokenEntity._id)}` : `${body.appId}:${ip}`;
        ...
        const messages = [
            {
                role: "system" as const,
                content: ["You are AutoUI assistant.",
